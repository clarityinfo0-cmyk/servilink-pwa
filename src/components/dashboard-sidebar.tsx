...
export function DashboardSidebar({ user }: { user: MockUser & { profileImageUrl?: string, verificationStatus?: string } }) {
  ...
  const navItems = [
    ...
    { title: "Mi Casa (Historial)", icon: Home, href: "/dashboard/client/history", roles: ['client'] },
    ...
    { title: "Gestionar Usuarios", icon: Users, href: "/dashboard/admin/users", roles: ['admin'] },
    { title: "Mi Perfil", icon: UserCircle, href: "/dashboard/profile", roles: ['client', 'technician'] }
  ];

  // Mensaje profesional y limpio para soporte
  const WHATSAPP_SUPPORT_URL = "https://wa.me/526873675477?text=Hola,%20solicito%20asistencia%20profesional%20con%20mi%20plataforma%20ServiLink%20Pro.";
    ...
    <Sidebar className="border-r border-sidebar-border bg-sidebar text-sidebar-foreground">
      <SidebarHeader className="p-4 flex flex-row items-center gap-2 border-b border-sidebar-border">
        ...
        <div className="flex flex-col">
          <span className="font-headline font-bold text-lg leading-tight">ServiLink</span>
          <span className="text-[10px] uppercase tracking-wider text-accent-foreground/70">{user.role}</span>
        </div>
      </SidebarHeader>
        ...
        <div className="flex items-center gap-3 mb-4 px-2">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <span className="text-[10px] text-sidebar-foreground/60 truncate font-black uppercase tracking-tighter">
              {user.verificationStatus === 'verified' ? 'Verificado ⭐' : 'Estatus: ' + (user.verificationStatus || 'Normal')}
            </span>
          </div>
        </div>
        <SidebarMenuButton 
